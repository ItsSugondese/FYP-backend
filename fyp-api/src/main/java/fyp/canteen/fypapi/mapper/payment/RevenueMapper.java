...
public interface RevenueMapper {
    ...
    @Select("select \n" +
            "coalesce (sum(whole.\"dueAmount\"), 0) as \"leftToPay\", \n" +
            " coalesce(sum(whole.\"totalTransaction\"), 0)as \"revenue\", \n" +
            "coalesce (sum(whole.\"totalPaid\"), 0) as \"paidAmount\" \n" +
            "from (\n" +
            "\tselect sum(suu.\"dueAmount\") as \"dueAmount\", suu.user_id, sum(suu.\"paidAmount\") as \"totalPaid\",\n" +
            "\t (sum(suu.\"dueAmount\") + sum(suu.\"paidAmount\")) as \"totalTransaction\"from (\n" +
